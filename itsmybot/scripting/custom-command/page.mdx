...
Each custom command need to be created in the `scripting/custom-commands` folder in the bot's directory. It's support subfolders, so you can organize your commands as you want. Each command is a .yml file that contains the command's configuration. You can disable a command by adding a underscore at the beginning of the file name, for example: `_my-command.yml`.
